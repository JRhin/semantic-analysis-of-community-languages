...
def relative_representation(dataframe: pl.DataFrame,
    ...
    Args:
        dataframe : pl.DataFrame
            The dataframe containing the absolute representetion for each topic token.
        topics : list[str]
            A list of topics for which we perform the relative representation.
    ...
    for topic in topics:
        ...
        df = dataframe.filter(pl.col('Topic')==topic)

        # Get the tokens
        tokens = df['Token'].to_list()
        
        embeddings = []
        for anchor in tqdm(anchors, desc=topic):
        ...
        current = pl.concat([pl.DataFrame({'Topic': topic,
                                           'Token': tokens}),
...
def semantic_differences(dataframe: pl.DataFrame,
    ...
    """Get the pairwise semantic difference between topics.
    ...
    Returns:
        relative_reps : dict[str, pl.DataFrame]
            The dictionary with the pl.DataFrame containing the semantic difference for each combination of topics.
    """
    results = dict()
    for t1, t2 in list(combinations(topics, 2)):
        # Get the sub-dataframe for each topic
        t1_df = dataframe.filter(pl.col('Topic')==t1)
        t2_df = dataframe.filter(pl.col('Topic')==t2)
